...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/journal-entries/date/:date", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  // Cycle Tracking
  app.get("/api/cycle-tracking", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  app.get("/api/cycle-tracking/current", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  app.post("/api/cycle-tracking", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
